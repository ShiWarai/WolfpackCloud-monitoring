...
2. **Персональные токены** — уникальный токен для каждого робота, выдаётся после подтверждения привязки
3. **API Proxy** — роботы не имеют прямого доступа к InfluxDB, метрики отправляются через API
4. **8-значный код** — одноразовый код привязки с ограниченным временем жизни (15 минут)
5. **Валидация токенов** — API проверяет токен и статус робота перед записью метрик
6. **Отзыв доступа** — при удалении/деактивации робота токен перестаёт работать
